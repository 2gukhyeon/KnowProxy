...
from transformers import AutoTokenizer, AutoModelForSequenceClassification, Trainer, TrainingArguments, LlamaForSequenceClassification, LlamaConfig, T5ForConditionalGeneration, BitsAndBytesConfig
...
parser.add_argument('--lr', help='learning rate', type=float, required=False, default=2e-4) # 2e-4 -> Llama 3.2 1B 검증 성능 유사한 것을 코드 검증 완료.
...
# parser.add_argument('--lr_warmup_ratio', help='warmup step ratio, which is # of steps ("total steps * ratio")', type=float, required=False, default=0.03) # llama2
...
parser.add_argument('--is_lora', help='use LoRA', type=str, required=False, default='yes')
...
elif task in ["arc-e", "arc-h", "obqa", "ko-arc"]:
    num_labels = 4
    question_path = None
    answer_path = None
...
# special_tokens_dict['additional_special_tokens'] = token_list # llama 2 chat은 제외!
